...
//LiquidCrystal_I2C lcd(0x27, 16, 2); // My display is using address 0x27
...
#ifdef WITH_1602_I2C   // Init case 1602 with I2C 
//lcd.begin();
// replace by the Code to avoid call the library
// commands
...
uint8_t lcd_addr=LCD_I2C_ADDR; //0x27;
uint8_t lcd_cols=16;
uint8_t lcd_rows=2;
uint8_t lcd_charsize=LCD_5x8DOTS;
uint8_t lcd_backlightval = LCD_BACKLIGHT;
...
uint8_t lcd_displaymode=0;
  ...
  expanderWrite(lcd_backlightval); // reset expanderand turn backlight off (Bit 8 =1)
  delay(1000);
  ...
  //command(LCD_FUNCTIONSET | lcd_displayfunction);
  lcd_send((LCD_FUNCTIONSET | lcd_displayfunction),0);
  ...
  lcd_send((LCD_DISPLAYCONTROL | lcd_displaycontrol),0);
  ...
  lcd_send((LCD_ENTRYMODESET | lcd_displaymode),0);
  
  //home();
  lcd_send((LCD_RETURNHOME),0);  // set cursor position to zero
  delayMicroseconds(2000);  // this command takes a long time!
#endif  // end init 1602 I2C
   ...
   digitalWrite(HW_LCD_RES_PIN,LOW); // SPI mode, pinMode() & digitalWrite compatible.
   ...
   pinMode(HW_LCD_RES_PIN,INPUT_PULLUP);
 #else
   //HW_LCD_RES_PORT |= _BV(HW_LCD_RES_PIN);	// switch RES to VCC
   digitalWrite(HW_LCD_RES_PIN,HIGH);
 #endif  /* LCD_SPI_OPEN_COL */
...
// unsigned char options, unsigned char width, unsigned char height) 
  // support for color display
   ...
   { 
  #if (LCD_ST7565_H_FLIP == 1)
      xx = ((SCREEN_WIDTH / (ONE_B/8)) - x - width);
  #else
      xx = x;
  #endif
      for (offset = 0; offset < width; offset++)
